...
const sha1EnabledCount = document.getElementById('sha1-enabled-count');
const sha1DisabledCount = document.getElementById('sha1-disabled-count');
const enabledTabCount = document.getElementById('enabled-tab-count');
const disabledTabCount = document.getElementById('disabled-tab-count');
...
async function init() {
  try {
    ...
    if (data.configured) setConnected(data.server);
  } catch (_) {}
}
...
function setConnected(serverUrl) {
  ...
  bootstrap.Collapse.getOrCreateInstance(document.getElementById('connection-collapse')).hide();
}
...
configForm.addEventListener('submit', async e => {
  ...
  const body = {
    ...
    api_token: authType === 'token' ? (document.getElementById('api-token').value.trim() || null) : null,
    client_id: authType === 'credentials' ? (document.getElementById('client-id').value.trim() || null) : null,
    client_secret: authType === 'credentials' ? (document.getElementById('client-secret').value.trim() || null) : null,
  };
  try {
...
scanBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    scanStatus.textContent = `Checked ${data.total_certs} certificate(s) — found ${data.sha1_count} SHA-1 (${data.enabled_sha1_count} enabled, ${data.disabled_sha1_count} disabled)`;
    certData = data.certs;
    renderResults(data);
  } catch (err) {
...
function updateCounts() {
  const enabled = certData.filter(cert => cert.enabled === true).length;
  const disabled = certData.filter(cert => cert.enabled === false).length;
  sha1Count.textContent = certData.length;
  sha1EnabledCount.textContent = enabled;
  sha1DisabledCount.textContent = disabled;
  enabledTabCount.textContent = enabled;
  disabledTabCount.textContent = disabled;
}
...
function setActiveTab(status) {
  ...
  enabledTab.classList.toggle('active', status === 'enabled');
  disabledTab.classList.toggle('active', status === 'disabled');
  enabledTab.setAttribute('aria-selected', status === 'enabled' ? 'true' : 'false');
  disabledTab.setAttribute('aria-selected', status === 'disabled' ? 'true' : 'false');
  renderCertificateTable();
}

enabledTab.addEventListener('click', () => setActiveTab('enabled'));
disabledTab.addEventListener('click', () => setActiveTab('disabled'));

function visibleCerts() {
  const enabled = activeStatus === 'enabled';
  return certData.filter(cert => cert.enabled === enabled);
}
...
function renderCertificateTable() {
  ...
  visibleCerts().forEach(cert => {
    ...
    tr.innerHTML = `
      <td class="ps-3"><input type="checkbox" class="form-check-input cert-checkbox" data-id="${cert.id}"></td>
      <td><span class="d-inline-block text-truncate" style="max-width:220px" title="${subject}">${subject}</span></td>
      <td><span class="d-inline-block text-truncate text-muted small" style="max-width:200px" title="${issuer}">${issuer}</span></td>
      <td>${expiry}</td>
      <td><span class="small">${usage}</span></td>
      <td>${badge}</td>
      <td class="pe-3"><button class="btn btn-outline-secondary btn-sm details-btn py-0 px-2" data-id="${cert.id}"><i class="bi bi-info-circle me-1"></i>Details</button></td>`;
    certsTbody.appendChild(tr);
  });
  certsTbody.querySelectorAll('.cert-checkbox').forEach(cb => cb.addEventListener('change', syncSelection));
  certsTbody.querySelectorAll('.cert-row').forEach(row => {
    row.addEventListener('click', e => {
      if (e.target.closest('.details-btn') || e.target.classList.contains('cert-checkbox')) return;
      const cb = row.querySelector('.cert-checkbox');
      cb.checked = !cb.checked;
      syncSelection();
    });
  });
  certsTbody.querySelectorAll('.details-btn').forEach(btn => btn.addEventListener('click', e => { e.stopPropagation(); showDetails(btn.dataset.id); }));
  syncSelection();
}
...
function syncSelection() {
  ...
  selectionLabel.textContent = n > 0 ? `${n} of ${all.length} visible selected` : '';
}

selectAllCb.addEventListener('change', () => {
  certsTbody.querySelectorAll('.cert-checkbox').forEach(cb => { cb.checked = selectAllCb.checked; });
  syncSelection();
});
...
deleteBtn.addEventListener('click', () => {
  ...
  selectedIds.forEach(id => {
    ...
    const subject = cert ? esc(cert.subject || cert.subject_dn || cert.common_name || `ID ${id}`) : `ID ${id}`;
    const li = document.createElement('li');
    li.className = 'list-group-item py-2 small';
    li.innerHTML = `<i class="bi bi-file-earmark-x text-danger me-2"></i>${subject}<span class="text-muted ms-1">(ID: ${id})</span>`;
    list.appendChild(li);
  });
  deleteModal.show();
});
...
confirmBtn.addEventListener('click', async () => {
  ...
  try {
    const res = await fetch('/api/certs/delete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids }) });
    const data = await res.json();
    if (!res.ok) throw new Error(data.detail || 'Delete failed');
    data.deleted.forEach(id => { certData = certData.filter(c => String(c.id) !== String(id)); });
    selectedIds.clear();
    updateCounts();
    ...
    if (fail === 0) showToast('success', `Successfully deleted ${ok} certificate(s).`);
    else { showToast('error', `Deleted ${ok}, failed ${fail}. See browser console.`); console.error('Failed deletions:', data.failed); }
  } catch (err) {
...
function showDetails(id) {
  ...
  const rows = Object.entries(cert).filter(([k]) => !SKIP.has(k)).map(([k, v]) => {
    const display = k === '_detected_by' ? detectionBadge(v) : `<span class="font-monospace small">${esc(String(v ?? '—'))}</span>`;
    return `<tr><td class="text-secondary">${esc(k)}</td><td style="word-break:break-all">${display}</td></tr>`;
  }).join('');
  document.getElementById('detail-body').innerHTML = `<table class="table table-sm detail-table mb-0">${rows}</table>`;
  detailModal.show();
}
...
function formatExpiry(raw) {
  ...
  } catch (_) { return `<span class="small">${esc(raw)}</span>`; }
}
